...
{
    if (reg_id == K3_CTRL_REG_ID && reg_type == 1) { // only when reg_type is SPR then update the CTRL register
        memcpy_s(buf, sizeof(uint64_t), &g_k3CtrlValue, sizeof(uint64_t));
    }
    return true;
}
...
{
    if (reg_id == K3_CTRL_REG_ID && reg_type == 1) { // only when reg_type is SPR then update the CTRL register
        memcpy_s(&g_k3CtrlValue, sizeof(uint64_t), buf, sizeof(uint64_t));
    }
    return true;
}
