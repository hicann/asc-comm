...
{
    type_ = ccu_rep_type::shr;
    instr_count_ = ins_gen_ptr->get_instr_count(type_);
}

ccu_rep_sh_r::ccu_rep_sh_r(ccu_ins_generater_base* ins_gen_ptr, const variable& var_d, const variable& var_m)
...
ccu_rep_sh_r::ccu_rep_sh_r(ccu_ins_generater_base* ins_gen_ptr, const address& addr_d, const variable& var_m)
...
bool ccu_rep_sh_r::translate(ccu_kernel* ccu_kernel, ccu_instr*& instr, uint16_t& cur_instr_id, const trans_dep& dep)
{
    asc::check_ccu_not_null(instr, "[CcuRepShR::translate] instr is nullptr!");
    this->instr_id_ = cur_instr_id;
    translated_ = true;
    instr_count_ = ins_gen_ptr_->get_instr_count(type_);
    CHK_PRT_THROW(
        ins_gen_ptr_->ccu_rep_sh_r_translate(ccu_kernel, instr, this, dep) != CcuResult::CCU_SUCCESS,
        HCCL_ERROR("[CcuRepShR][translate] failed to translate for instrId[%u]", instr_id_), CcuResult::CCU_E_INTERNAL,
        "CcuRepShR translate failed");
    CHK_PRT_THROW(
...
std::string ccu_rep_sh_r::describe()
{
    if (shift_type_ == shift_type::logical_shift) {
        switch (sub_type_) {
