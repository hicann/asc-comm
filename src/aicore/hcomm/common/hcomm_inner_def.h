...
#ifndef IMPL_COMM_API_AICORE_HCOMM_COMMON_HCOMM_INNER_DEF_H
#define IMPL_COMM_API_AICORE_HCOMM_COMMON_HCOMM_INNER_DEF_H
...
static constexpr UrmaWqeEntry URMA_INLINE_CFG = {
    .odr = 5,
    .fence = 1,
    .se = 0,
    .cqe = 1,
...
typedef struct {
    ...
    uint32_t sqHead; // SQ producer index in WQE BBs
    uint32_t sqTail; // Submitted WQE count, reserved for future use
    uint32_t cqHead; // Number of expected CQEs
    uint32_t cqTail; // Number of consumed CQEs
    uint8_t reserve[144];
} ChannelEntity;
...
#endif // IMPL_COMM_API_AICORE_HCOMM_COMMON_HCOMM_INNER_DEF_H
