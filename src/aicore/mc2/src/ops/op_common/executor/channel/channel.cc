    ...
    u32 netLayer, const std::string &tag, bool linkRequired)
{
    ...
    for (u32 rank: subcommInfo[COMM_LEVEL0]) {
        ...
        if (channels.size() == channelCountBefore) {
            if (linkRequired) {
                HCCL_ERROR("[%s] No matching protocol intra-frame link between myRank=%u and rank=%u.",
                    tag.c_str(), myRank, rank);
                return HcclResult::HCCL_E_INTERNAL;
            }
            HCCL_WARNING("[%s] No matching protocol inter-frame link between myRank=%u and rank=%u.",
                tag.c_str(), myRank, rank);
        }
    }
#else
