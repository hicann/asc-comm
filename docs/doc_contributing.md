...
| 构建与依赖 | 构建脚本、CMake入口、三方依赖说明 | `docs/guide/` |
| API参考 | 接口功能、原型、参数、返回值和约束 | `docs/api/` |
| 使用指南 | API调用流程、协议能力和注意事项 | `docs/guide/` |
| 样例说明 | 样例目录入口、运行边界和验证说明 | `examples/` |
...
README.md ──快速上手──→ docs/quick_start.md
          ──构建测试──→ docs/guide/build_and_test.md
          ──API参考───→ docs/api/
          ──样例入口──→ examples/README.md

docs/guide/ ──首次引入API──→ docs/api/
             ──涉及样例────→ examples/README.md

docs/api/ ───使用流程──→ docs/guide/
