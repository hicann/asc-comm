...
通过指定通信组team和对端rank，提交单边写任务，将本端对称window中的数据写入对端对称window。
...
- 当前只支持`COMM_PROTOCOL_UB_CTP`协议路径。单次调用`Put`的入参`bytes`取值范围为`0 < bytes <= 256 * 1024 * 1024`。
    ...
    uint32_t rankNum, uint32_t rankId)
{
    ...
    // 单边写：本端srcWin数据写入对端dstWin，写完成后对远端signal原子加1
