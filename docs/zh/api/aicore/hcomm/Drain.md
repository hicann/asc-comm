...
| `batchHandle` | 输入/输出 | `MakeBatchHandle`创建的批量句柄。等待成功后，句柄缓存的CQ tail及通道实体中的CQ tail会更新。 |
...
| `T` | 批量句柄类型，由`batchHandle`实参推导；仅用于批量重载，当前支持`UbcCtpBatchHandle`。 |
...
- BatchHandle使用期间需要独占对应通道，不能在同一通道上混用普通`Drain`。
