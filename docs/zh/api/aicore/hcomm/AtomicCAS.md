...
提交Compare-and-swap原子操作任务，比较远端地址`dst`中的值和`compareVal`，相等时将远端值替换为`swapVal`，并将替换前的旧值写入本端`fetchAddr`。
...
| `fetchAddr` | 输出 | 本端GM地址，用于保存远端地址执行CAS前的旧值。 |
| `compareVal` | 输入 | 比较值。 |
| `swapVal` | 输入 | 比较成功时写入远端目标地址的新值。 |
...
- 单个`AtomicCAS`任务在URMA SQ中占用2个WQE block。
