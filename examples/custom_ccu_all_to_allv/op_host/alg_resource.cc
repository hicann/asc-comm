...
static HcclResult RegisterAllToAllVKernel(HcclComm comm, const OpParam &param, AlgResourceCtx &resCtxHost,
                                          const std::vector<ChannelHandle> &kernelChannels)
{
    CcuKernelInfo kernelInfo;
    strcpy_s(kernelInfo.kernelFuncName, sizeof(kernelInfo.kernelFuncName), "CcuAllToAllVMesh1DKernel");
...
HcclResult AllocAlgResource(HcclComm comm, const OpParam &param, AlgResourceCtx &resCtxHost)
{
    ...
    RETURN_IF_HCCL_FAIL(RegisterAllToAllVKernel(comm, param, resCtxHost, kernelChannels));
    return HCCL_SUCCESS;
}
