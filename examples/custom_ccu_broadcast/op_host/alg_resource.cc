...
static HcclResult RegisterBroadcastKernel(HcclComm comm, const OpParam &param, AlgResourceCtx &resCtxHost,
                                          const std::vector<ChannelHandle> &kernelChannels)
{
    ...
    kernelArg->rootId = param.rootRank;
...
HcclResult AllocAlgResource(HcclComm comm, const OpParam &param, AlgResourceCtx &resCtxHost)
{
    ...
    RETURN_IF_HCCL_FAIL(RegisterBroadcastKernel(comm, param, resCtxHost, kernelChannels));
    return HCCL_SUCCESS;
}
