...
static HcclResult RegisterAllToAllVCKernel(HcclComm comm, const OpParam &param, AlgResourceCtx &resCtxHost,
                                          const std::vector<ChannelHandle> &kernelChannels)
{
    CcuKernelInfo kernelInfo;
    strcpy_s(kernelInfo.kernelFuncName, sizeof(kernelInfo.kernelFuncName), "CcuAllToAllVCMesh1DKernel");
    kernelInfo.kernelFunc = reinterpret_cast<void *>(CcuAllToAllVCMesh1DKernel);

    auto kernelArg = std::make_shared<CcuKernelArgAllToAllVCMesh1D>();
...
HcclResult AllocAlgResource(HcclComm comm, const OpParam &param, AlgResourceCtx &resCtxHost)
{
    ...
    RETURN_IF_HCCL_FAIL(RegisterAllToAllVCKernel(comm, param, resCtxHost, kernelChannels));
    return HCCL_SUCCESS;
}
