...
// uint64_t goSize0, uint64_t goSize1, uint64_t goSize2, uint64_t goSize3,  CcuKernelArg arg)
// {
//     auto *kernelArg = static_cast<CcuKernelArgAllGatherMesh1DMem2Mem *>(arg);
...
//     // 2.加载参数
//     CCU_CHK_RET(ccu::LoadArg(ctx.input, CCU_GET_ARG_IDX(inputAddr)));
//     CCU_CHK_RET(ccu::LoadArg(ctx.output[ctx.arg->rankId], CCU_GET_ARG_IDX(outputAddr)));
//     CCU_CHK_RET(ccu::LoadArg(ctx.token[ctx.arg->rankId], CCU_GET_ARG_IDX(token)));
//     CCU_CHK_RET(ccu::LoadArg(ctx.currentRankSliceInputOffset, CCU_GET_ARG_IDX(currentRankSliceInputOffset)));
//     CCU_CHK_RET(ccu::LoadArg(ctx.currentRankSliceOutputOffset, CCU_GET_ARG_IDX(currentRankSliceOutputOffset)));
//     CCU_CHK_RET(ccu::LoadArg(ctx.goSize.addrOffset, CCU_GET_ARG_IDX(goSize0)));
//     CCU_CHK_RET(ccu::LoadArg(ctx.goSize.loopParam, CCU_GET_ARG_IDX(goSize1)));
//     CCU_CHK_RET(ccu::LoadArg(ctx.goSize.parallelParam, CCU_GET_ARG_IDX(goSize2)));
//     CCU_CHK_RET(ccu::LoadArg(ctx.goSize.residual, CCU_GET_ARG_IDX(goSize3)));
//     CCU_CHK_RET(ccu::LoadArg(ctx.sliceSize, CCU_GET_ARG_IDX(sliceSize))); //  CCU_GET_ARG_IDX(sliceSize)=5
