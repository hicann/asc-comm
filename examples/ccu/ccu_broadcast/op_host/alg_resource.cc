...
static HcclResult RegisterBroadcastKernel(
    ...
    uint32_t kernelDie)
{
    ...
    if (strcpy_s(kernelInfo.kernel_func_name, sizeof(kernelInfo.kernel_func_name), "CcuBroadcastMesh1DKernel") != EOK) {
        return HCCL_E_INTERNAL;
    }
    kernelInfo.kernel_func = reinterpret_cast<void*>(CcuBroadcastMesh1DKernel);

    auto kernel_arg = std::make_shared<CcuKernelArgBroadcastMesh1D>();
    kernel_arg->rankSize = param_.rankSize;
    kernel_arg->rankId = param_.myRank;
    kernel_arg->rootId = param_.rootRank;
...
HcclResult AllocAlgResource(HcclComm comm, const OpParam& param_, AlgResourceCtx& resCtxHost)
{
    ...
    RETURN_IF_HCCL_FAIL(RegisterBroadcastKernel(comm, param_, resCtxHost, kernelChannels, kernelDie));
    return HCCL_SUCCESS;
}
