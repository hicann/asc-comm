...
static HcclResult RegisterAllToAllVKernel(
    ...
    uint32_t kernelDie)
{
    ...
    if (strcpy_s(kernelInfo.kernel_func_name, sizeof(kernelInfo.kernel_func_name), "CcuAllToAllVMesh1DKernel") != EOK) {
        return HCCL_E_INTERNAL;
    }
    kernelInfo.kernel_func = reinterpret_cast<void*>(CcuAllToAllVMesh1DKernel);

    auto kernel_arg = std::make_shared<CcuKernelArgAllToAllVMesh1D>();
...
HcclResult AllocAlgResource(HcclComm comm, const OpParam& param_, AlgResourceCtx& resCtxHost)
{
    ...
    RETURN_IF_HCCL_FAIL(RegisterAllToAllVKernel(comm, param_, resCtxHost, kernelChannels, kernelDie));
    return HCCL_SUCCESS;
}
