...
并申请通信资源上下文。随后，AICore `all_gather_kernel`通过`Hccl::AllGather`将各rank输入
按照rank id顺序聚合到每个rank的`recvBuf`。
...
- 配置环境变量
  ...
  `Mc2SetCcCommEngine`设置`CCU_SCHED`。
...
- 执行结果
  ...
  每个rank均完成AllGather，并输出聚合后的`recvBuf`，表示样例执行成功。
