    ...
    uint32_t& kernelDie)
{
    ...
    for (uint32_t remoteRank = 0; remoteRank < param_.rankSize; remoteRank++) {
        ...
        RETURN_IF_HCCL_FAIL(
            HcclChannelAcquire(comm, CommEngine::COMM_ENGINE_CCU, &desc, 1, &kernelChannels[channelIndex]));
        channelIndex++;
    }
    ...
    uint32_t kernelDie)
{
    ...
    if (strcpy_s(kernelInfo.kernel_func_name, sizeof(kernelInfo.kernel_func_name), "CcuAllGatherMesh1DMem2MemKernel") !=
        EOK) {
    ...
    CcuResult reg_ret = asccomm_ccu_kernel_register(
        ...
        kernel_args, kernel_arg_num, &kernelHandle);
