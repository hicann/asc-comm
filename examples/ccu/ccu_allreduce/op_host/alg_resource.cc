...
static HcclResult RegisterAllReduceKernel(
    ...
    uint32_t kernelDie)
{
    ...
    if (strcpy_s(kernelInfo.kernel_func_name, sizeof(kernelInfo.kernel_func_name), "CcuAllReduceMesh1DMem2MemKernel") !=
        EOK) {
        return HCCL_E_INTERNAL;
    }
    kernelInfo.kernel_func = reinterpret_cast<void*>(CcuAllReduceMesh1DMem2MemKernel);

    auto kernel_arg = std::make_shared<CcuKernelArgAllReduceMesh1DMem2Mem>();
...
HcclResult AllocAlgResource(HcclComm comm, const OpParam& param_, AlgResourceCtx& resCtxHost)
{
    ...
    RETURN_IF_HCCL_FAIL(RegisterAllReduceKernel(comm, param_, resCtxHost, kernelChannels, kernelDie));
    return HCCL_SUCCESS;
}
