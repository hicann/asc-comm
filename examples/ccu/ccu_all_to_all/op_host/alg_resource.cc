...
static HcclResult RegisterAllToAllKernel(
    ...
    uint32_t kernelDie)
{
    ...
    if (strcpy_s(kernelInfo.kernel_func_name, sizeof(kernelInfo.kernel_func_name), "CcuAllToAllMesh1DMem2MemKernel") !=
        EOK) {
        return HCCL_E_INTERNAL;
    }
    kernelInfo.kernel_func = reinterpret_cast<void*>(CcuAllToAllMesh1DMem2MemKernel);

    auto kernel_arg = std::make_shared<CcuKernelArgAllToAllMesh1DMem2Mem>();
...
HcclResult AllocAlgResource(HcclComm comm, const OpParam& param_, AlgResourceCtx& resCtxHost)
{
    ...
    RETURN_IF_HCCL_FAIL(RegisterAllToAllKernel(comm, param_, resCtxHost, kernelChannels, kernelDie));
    return HCCL_SUCCESS;
}
