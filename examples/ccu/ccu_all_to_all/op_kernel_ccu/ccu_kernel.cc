...
static void InitGroupCopyResources(
    AllToAllMesh1DMem2MemContext& ctx, ccu::local_addr* loopSrc, ccu::local_addr* loopDst, ccu::variable* loopLen)
{
...
static CcuResult GroupCopy(
    AllToAllMesh1DMem2MemContext& ctx, ccu::local_addr dst_, ccu::local_addr src_, GroupOpSizeVars goSize)
{
    ...
    CCU_IF(ctx.sliceSize != 0)
    {
        ...
        for (uint64_t rankIdx = 0; rankIdx < ctx.arg->rankSize; rankIdx++) {
            ...
            if (rankIdx != ctx.arg->rankId) {
                ...
                RETURN_IF_CCU_FAIL(ccu::Write(
                    ctx.arg->channels[channelId], remoteDst[rankIdx], remoteRankSrc, ctx.sliceSize, ctx.event, mask_));
                totalMask |= mask_;
                channelId++;
            }
        }
...
CcuResult CcuAllToAllMesh1DMem2MemKernel(ccu_kernel_arg arg)
{
    auto* kernel_arg = static_cast<CcuKernelArgAllToAllMesh1DMem2Mem*>(arg);
    ...
    // 4. 执行算法
    RETURN_IF_CCU_FAIL(ExecuteAllToAllTransfer(ctx));
